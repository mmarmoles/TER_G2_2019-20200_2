...
def CifradoVigenere():
    ...
    #    for letra in each_split:
    #        numero = ((letra_indice[letra] + letra_indice[dato.clave[i]]) % len(abcdario))
    #        if indice_letra[numero] != 32 and indice_letra[numero] != 44 and indice_letra[numero] != 46:
    #            dato.textoVigenere += indice_letra[numero]
    #        i += 1
    #print("Mensaje Vigenere: ", dato.textoVigenere)

    print("Encriptando")
    dato.textoGuardado, dato.clave = msg_and_key()
    cipher_encryption(dato.textoGuardado, dato.clave)
...
def DescifradoVigenere():
    split_cipher = [dato.textoGuardado [i:i + len (dato.clave)] for i in range(0, len(dato.textoGuardado), len(dato.clave))]
    for each_split in split_cipher:
        i = 0
        for letra in each_split:
            numero = (letra_indice[letra] - letra_indice[key[i]]) % len(abcdario)
            decrypted += indice_letra[numero]
            i+=1
            if letra_indice[key[i]] == " ":
                print("MEEEEEEEEEEEEEEEEEEEC !!!")
...
# VARIABLES STATICAS
#abcdario = 'ABCDEFGHIJKLMNÑOPQRSTUVWXYZ0987654321:;()/\_-!#"<>¿?@abcdefghijklmnñopqrstuvwxyz'
#abcdario = 'ABCDEFGHIJKLMNÑOPQRSTUVWXYZ 0987654321.:,;()/\_-!#"<>¿?@abcdefghijklmnñopqrstuvwxyz'
#abcdario = 'ABCDEFGHIJKLMNÑOPQRSTUVWXYZ 0987654321.,'
